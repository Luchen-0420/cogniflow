...
优先级 2：语义匹配（使用 GLM Embedding API 进行向量检索）
优先级 3：关系网络（被匹配内容引用或关联的其他内容）
```
...
**新增表：knowledge_links（知识关联表）**
```sql
CREATE TABLE knowledge_links (
  id SERIAL PRIMARY KEY,
  source_item_id INT REFERENCES items(id),     -- 源卡片
  target_item_id INT REFERENCES items(id),     -- 关联的卡片
  link_type VARCHAR(20),                        -- 关联类型：auto/manual
  relevance_score DECIMAL(3,2),                 -- 相关度评分 0-1
  created_at TIMESTAMP DEFAULT NOW(),
  created_by INT REFERENCES users(id)
);

CREATE INDEX idx_source_item ON knowledge_links(source_item_id);
CREATE INDEX idx_target_item ON knowledge_links(target_item_id);
```

**新增表：research_suggestions（调研建议记录）**
```sql
CREATE TABLE research_suggestions (
  id SERIAL PRIMARY KEY,
  item_id INT REFERENCES items(id),
  suggestion_type VARCHAR(50),                  -- gap/update/external
  suggestion_content JSONB,                     -- 建议详情
  user_action VARCHAR(20),                      -- accepted/ignored/pending
  created_at TIMESTAMP DEFAULT NOW()
);
```
...
**端点 1：获取智能推荐**
```typescript
POST /api/items/:id/smart-assist
Request Body:
{
  "content": "调研 AI Agent 产品的竞品分析",
  "layers": ["history", "gap", "external"]  // 可选择层级
}
...
**端点 2：批量关联资料**
```typescript
POST /api/items/:id/link-resources
Request Body:
{
...
**语义检索（使用 GLM Embedding）**
```typescript
async function semanticSearch(query: string, userId: number) {
  // 1. 获取查询向量
  const queryEmbedding = await getEmbedding(query);
  
  // 2. 从数据库获取用户所有卡片的向量（预先计算）
  const userItems = await db.query(`
    SELECT id, title, content, embedding_vector
    FROM items WHERE user_id = $1
  `, [userId]);
  
  // 3. 计算余弦相似度
  const results = userItems.map(item => ({
    ...item,
    similarity: cosineSimilarity(queryEmbedding, item.embedding_vector)
  }));
...
你是知识管理专家。用户要调研「${topic}」，他的知识库如下：

已有笔记（${existingNotes.length} 条）：
${existingNotes.map((n, i) => 
  `${i+1}. ${n.title}\n   摘要：${n.summary}\n   时间：${n.date}`
).join('\n\n')}

请分析：
1. 知识结构完整度（0-100分）：缺少哪些关键调研维度？
2. 时效性：最新资料距今多久？在该领域是否已过时？
3. 深度：是浅层资料收集还是深度分析？缺少什么？
4. 给出 3-5 条具体建议，每条包括：要补充什么、为什么、优先级

返回 JSON 格式（严格遵循以下 schema）：
{
  "completeness": {
    "score": number,
  ...
  "suggestions": [{"action": string, "details": string}],
  "outline": string[]
}
`;
```
...
**策略 1：预计算向量**
- 卡片创建/修改时立即计算 embedding
- 存储在数据库，避免实时计算

**策略 2：分层加载**
- 第一层（历史关联）立即返回（使用数据库索引）
- 第二层（缺口分析）异步加载，用 WebSocket 推送
- 第三层（外部推荐）用户可选，避免浪费 API 调用
