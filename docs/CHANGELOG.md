...
#### 登录状态持久化问题修复
- **修复**: 用户刷新页面后丢失登录状态的问题
- **优化**: Token 有效期从 7 天延长到 30 天
- **新增**: Token 自动刷新机制，避免过期
- **改进**: API 请求失败时自动刷新 token 并重试
- **影响文件**:
  - `server/routes/users.ts` - 延长 token 有效期，新增刷新端点
  - `src/db/postgresAuth.ts` - 优化初始化验证逻辑
  - `src/db/postgresApi.ts` - 添加智能重试机制
  - `docs/fixes/AUTH_TOKEN_PERSISTENCE_FIX.md` - 详细修复说明

**三层防护机制**：
1. ✅ **延长有效期**：Token 从 7 天延长到 30 天
2. ✅ **自动刷新**：页面初始化时智能刷新过期 token
3. ✅ **智能重试**：API 请求收到 401 时自动刷新并重试

**用户体验改进**：
- 30 天内无需重新登录（之前只有 7 天）
- Token 过期时自动续期，用户无感知
- 网络错误时保留登录状态，恢复后继续使用
- "能看不能改"的问题得到根本解决

#### 冲突检测逻辑优化
- **修复**: 已归档、已完成、过期的事项不应参与冲突检测
- **优化**: 引入"活跃事项"概念，只有活跃的事项才会产生冲突
- **改进**: 冲突检测更精准，减少误报
- **性能**: 减少不必要的比较计算，提升检测效率
- **影响文件**:
  - `server/routes/items.ts` - 更新冲突检测查询条件
  - `docs/fixes/CONFLICT_DETECTION_OPTIMIZATION.md` - 详细说明文档
  - `scripts/test-conflict-optimization.sh` - 测试脚本
